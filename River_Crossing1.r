...
# Comment:    Solves the Farmer-Wolf-Goat-Cabbage River Crossing Problem
################################################################
...
checkValidity <- function(S){
  ...
  if (sum(S[1],S[2])==2 & S[4]==0) {result=0}
  if (sum(S[2],S[3])==2 & S[4]==0) {result=0}
  if (sum(S[5],S[6])==2 & S[8]==0) {result=0}
  if (sum(S[6],S[7])==2 & S[8]==0) {result=0}
...
Start_State=data.frame(W_L=1,G_L=1,C_L=1,F_L=1,W_R=0,G_R=0,C_R=0,F_R=0)
Success_State= data.frame(W_L=0,G_L=0,C_L=0,F_L=0,W_R=1,G_R=1,C_R=1,F_R=1)
Acceptable_states<-c()
